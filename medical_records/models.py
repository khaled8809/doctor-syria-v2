...
class Radiology(models.Model):
    ...
    radiology_type = models.CharField(
        max_length=20,
        choices=RADIOLOGY_TYPES,
        verbose_name=_('Radiology Type')
    )
...
class InventoryTransaction(models.Model):
    ...
    def __str__(self):
        return f"{self.get_transaction_type_display()} - {self.inventory.medication.name}"
...
class InvoiceItem(models.Model):
    ...
    def clean(self):
        ...
        if self.quantity * self.unit_price != self.total:
            raise ValidationError(_('Total must be equal to quantity multiplied by unit price'))
...
class Payment(models.Model):
    ...
    def clean(self):
        ...
        if self.amount > self.invoice.total:
            raise ValidationError(_('Payment amount cannot be greater than invoice total'))
        
        total_payments = sum(payment.amount for payment in self.invoice.payments.exclude(id=self.id))
    ...
    def save(self, *args, **kwargs):
        ...
        # تحديث حالة الفاتورة
        total_payments = sum(payment.amount for payment in self.invoice.payments.filter(status='completed'))
...
class InsuranceClaim(models.Model):
    ...
    def clean(self):
        ...
        if self.amount_claimed > self.invoice.total:
            raise ValidationError(_('Claimed amount cannot be greater than invoice total'))
        
        if self.amount_approved and self.amount_approved > self.amount_claimed:
            raise ValidationError(_('Approved amount cannot be greater than claimed amount'))
        
        if self.status == 'rejected' and not self.rejection_reason:
            raise ValidationError(_('Rejection reason is required when claim is rejected'))
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:  # إذا كانت مطالبة جديدة
            if not self.amount_claimed:
                self.amount_claimed = self.insurance.calculate_coverage(self.invoice.total)
