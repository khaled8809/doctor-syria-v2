...
from ..models import Hospital, Department, Doctor, Admission, Transfer, EmergencyCase, Patient

class HospitalService:
    @staticmethod
    def get_available_hospitals(specialty: Optional[str] = None, city: Optional[str] = None) -> List[Hospital]:
        """
    ...
    @staticmethod
    def get_doctor_schedule(doctor_id: int, date: datetime) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        return {
            ...
            'appointments': Admission.objects.filter(
                doctor=doctor,
                admission_date__date=date.date()
    ...
    @staticmethod
    def discharge_patient(admission_id: int, discharge_date: datetime, notes: str = '') -> Admission:
        """
    ...
    @staticmethod
    def get_hospital_statistics(hospital_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'emergency_cases': {
                ...
                'outcomes': {
                    ...
                    'transferred': emergency_cases.filter(outcome='TRANSFERRED').count(),
                    'deceased': emergency_cases.filter(outcome='DECEASED').count(),
                }
            },
            'bed_utilization': (hospital.bed_capacity - hospital.available_beds) / hospital.bed_capacity * 100,
            'average_stay': admissions.filter(
                discharge_date__isnull=False
            ).annotate(
                stay_duration=F('discharge_date') - F('admission_date')
