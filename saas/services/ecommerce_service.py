...
class ECommerceService:
    ...
    def add_to_cart(
        ...
        if quantity < product.min_order_quantity:
            raise ValidationError(f"Minimum order quantity is {product.min_order_quantity}")
        
        if product.max_order_quantity and quantity > product.max_order_quantity:
            raise ValidationError(f"Maximum order quantity is {product.max_order_quantity}")
    ...
    def get_cart(user_id: int) -> Cart:
        ...
        return Cart.objects.prefetch_related(
            'items__product'
        ).get_or_create(user_id=user_id)[0]
    ...
    def update_cart_item(cart_item_id: int, quantity: int) -> CartItem:
        ...
        if quantity < cart_item.product.min_order_quantity:
            raise ValidationError(f"Minimum order quantity is {cart_item.product.min_order_quantity}")
        
        if cart_item.product.max_order_quantity and quantity > cart_item.product.max_order_quantity:
            raise ValidationError(f"Maximum order quantity is {cart_item.product.max_order_quantity}")
    ...
    def create_order(
        ...
        shipping_fee = Decimal('10.00') if total_amount < Decimal('100.00') else Decimal('0.00')
        ...
        for cart_item in cart.items.all():
            if not ECommerceService.check_product_availability(
                ...
                raise ValidationError(f"Product {cart_item.product.name} is not available in requested quantity")
