...
class EmergencyAdmissionFlowTest(APITestCase):
    ...
    def test_complete_emergency_flow(self):
        ...
        response = self.client.patch(
            reverse('emergencycase-detail', args=[emergency_id]),
            emergency_update
        )
        ...
        response = self.client.patch(
            reverse('admission-detail', args=[admission_id]),
            admission_complete
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class InventoryManagementFlowTest(APITestCase):
    ...
    def test_complete_inventory_flow(self):
        ...
        order_data = {
            ...
            'warehouse': self.warehouse.id,
            'status': 'DRAFT',
            'expected_delivery_date': (timezone.now().date() + timezone.timedelta(days=7)).isoformat(),
            'total_amount': 500.0,
        ...
        response = self.client.patch(
            reverse('inventoryitem-detail', args=[inventory_id]),
            inventory_update
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
