...
from .forms import (
    MedicineForm, InventoryForm, PrescriptionForm, PrescriptionItemForm,
    InventoryTransactionForm, InventorySearchForm, PrescriptionSearchForm
)

class PharmacistRequiredMixin(UserPassesTestMixin):
    def test_func(self):
        return self.request.user.is_staff or hasattr(self.request.user, 'pharmacist_profile')
    ...
    def get_queryset(self):
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['category']:
                queryset = queryset.filter(medicine__category=form.cleaned_data['category'])
    ...
    def get_queryset(self):
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['date_from']:
                queryset = queryset.filter(date_prescribed__gte=form.cleaned_data['date_from'])
            if form.cleaned_data['date_to']:
                queryset = queryset.filter(date_prescribed__lte=form.cleaned_data['date_to'])
...
class InventoryReportView(PharmacistRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # قيمة المخزون
        context['total_value'] = Inventory.objects.aggregate(
