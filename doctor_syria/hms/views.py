...
from .models import (
    Department, Ward, Room, Bed, Admission, Transfer,
    NursingRound, DoctorRound, Discharge, Equipment,
    MaintenanceRecord, InventoryItem
)
from .forms import (
    DepartmentForm, WardForm, RoomForm, BedForm,
    AdmissionForm, TransferForm, NursingRoundForm,
    DoctorRoundForm, DischargeForm, EquipmentForm,
    MaintenanceRecordForm, InventoryItemForm,
    AdmissionSearchForm, EquipmentSearchForm,
    InventorySearchForm, DateRangeForm
)
...
def admission_list(request):
    ...
    if form.is_valid():
        ...
        if form.cleaned_data['admission_date_from']:
            admissions = admissions.filter(admission_date__gte=form.cleaned_data['admission_date_from'])
        if form.cleaned_data['admission_date_to']:
            admissions = admissions.filter(admission_date__lte=form.cleaned_data['admission_date_to'])
    ...
    return render(request, 'hms/admission/list.html', {
...
def equipment_list(request):
    ...
    if form.is_valid():
        ...
        if form.cleaned_data['maintenance_due']:
            equipment = equipment.filter(next_maintenance__lte=models.timezone.now().date())
    
    return render(request, 'hms/equipment/list.html', {
        'equipment': equipment,
        'form': form
    })
...
def equipment_detail(request, pk):
    ...
    maintenance_records = equipment.maintenancerecord_set.all().order_by('-maintenance_date')
    return render(request, 'hms/equipment/detail.html', {
        'equipment': equipment,
...
def inventory_list(request):
    ...
    return render(request, 'hms/inventory/list.html', {
        'inventory': inventory,
        'form': form
    })
...
def occupancy_report(request):
    ...
    if form.is_valid():
        ...
        data['occupancy_rate'] = (
            ...
            if data['total_beds'] > 0 else 0
        )
    
    return render(request, 'hms/reports/occupancy.html', {
        'form': form,
        'data': data
    })
