...
class AdmissionSearchForm(forms.Form):
    ...
    admission_date_from = forms.DateField(required=False, widget=forms.DateInput(attrs={'type': 'date'}))
    admission_date_to = forms.DateField(required=False, widget=forms.DateInput(attrs={'type': 'date'}))
    status = forms.ChoiceField(choices=[('', '----')] + Admission.STATUS_CHOICES, required=False)
