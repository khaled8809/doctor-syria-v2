...
from pharmacy.models import Medicine, Order, OrderItem, Inventory, StockAlert, ExpiryAlert
...
class SoftDeleteModelSerializer(BaseModelSerializer):
    ...
    class Meta:
        abstract = True
        fields = BaseModelSerializer.Meta.fields + ['is_deleted', 'deleted_at', 'deleted_by']
...
class MedicineSerializer(SoftDeleteModelSerializer):
    ...
    storage_condition_display = serializers.CharField(source='get_storage_condition_display', read_only=True)
    ...
    class Meta:
        ...
        fields = SoftDeleteModelSerializer.Meta.fields + [
            'name', 'scientific_name', 'category', 'category_display',
            'form', 'form_display', 'manufacturer', 'description',
            'dosage', 'storage_condition', 'storage_condition_display',
            'price', 'requires_prescription', 'is_available', 'image',
            'barcode', 'expiry_date', 'side_effects', 'contraindications',
            'interactions', 'notes', 'is_expired', 'days_until_expiry',
            'inventory_quantity'
        ]
...
class OrderItemSerializer(BaseModelSerializer):
    ...
    class Meta:
        ...
        fields = BaseModelSerializer.Meta.fields + [
            'order', 'medicine', 'medicine_detail', 'quantity',
...
class OrderSerializer(BaseModelSerializer):
    ...
    class Meta:
        model = Order
        fields = BaseModelSerializer.Meta.fields + [
            'status', 'status_display', 'items', 'items_count',
            'total_amount', 'notes'
        ]
...
class InventorySerializer(BaseModelSerializer):
    ...
    class Meta:
        model = Inventory
        fields = BaseModelSerializer.Meta.fields + [
            'medicine', 'medicine_detail', 'quantity', 'minimum_quantity',
            'maximum_quantity', 'needs_restock', 'has_excess', 'notes'
        ]
...
class StockAlertSerializer(BaseModelSerializer):
    ...
    class Meta:
        model = StockAlert
        fields = BaseModelSerializer.Meta.fields + [
            'medicine', 'medicine_detail', 'alert_type', 'quantity',
            'threshold', 'is_resolved', 'notes'
        ]
...
class ExpiryAlertSerializer(BaseModelSerializer):
    ...
    class Meta:
        model = ExpiryAlert
        fields = BaseModelSerializer.Meta.fields + [
            'medicine', 'medicine_detail', 'expiry_date',
            'days_until_expiry', 'is_resolved', 'notes'
        ]
