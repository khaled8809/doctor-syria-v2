...
class BiometricManager:
    ...
    def register_fingerprint(self, fingerprint_data: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Fingerprint registration error for user {self.user.email}: {e}")
            return False
    ...
    def verify_fingerprint(self, fingerprint_data: str) -> bool:
        ...
        try:
            ...
            return hmac.compare_digest(
                test_hash.encode(),
                self.user.fingerprint_hash.encode()
            )
            
        except Exception as e:
            logger.error(f"Fingerprint verification error for user {self.user.email}: {e}")
            return False
    ...
    def verify_face(self, face_data: str, face_encoding: list, threshold: float = 0.6) -> bool:
        """
        التحقق من الوجه
        Verify face
    ...
    def verify_voice(self, voice_data: str, voice_features: list, threshold: float = 0.7) -> bool:
        """
        التحقق من الصوت
        Verify voice
        ...
        try:
            ...
            import numpy as np
            similarity = np.dot(
                np.array(voice_features),
                np.array(stored_features)
            ) / (
                np.linalg.norm(voice_features) *
    ...
    def clear_biometric_data(self) -> bool:
        ...
        """
        try:
            for attr in ['fingerprint_hash', 'fingerprint_salt',
            ...
            logger.error(f"Error clearing biometric data for user {self.user.email}: {e}")
            return False
