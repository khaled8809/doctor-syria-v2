...
class Medicine(TimestampMixin, SoftDeleteMixin, AuditMixin):
    ...
    image = models.ImageField(
        _("الصورة"), upload_to="medicines/", null=True, blank=True
    )
    barcode = models.CharField(
        _("الباركود"), max_length=50, unique=True, null=True, blank=True
    )
...
class Order(TimestampMixin, SoftDeleteMixin, AuditMixin):
    ...
    total_amount = models.DecimalField(
        _("المبلغ الإجمالي"), max_digits=10, decimal_places=2
    )
    discount = models.DecimalField(
        _("الخصم"), max_digits=10, decimal_places=2, default=0
    )
    insurance_coverage = models.DecimalField(
        _("تغطية التأمين"), max_digits=10, decimal_places=2, default=0
    )
    final_amount = models.DecimalField(
        _("المبلغ النهائي"), max_digits=10, decimal_places=2
    )
    notes = models.TextField(_("ملاحظات"), blank=True)
...
class OrderItem(TimestampMixin, AuditMixin):
    ...
    unit_price = models.DecimalField(
        _("سعر الوحدة"), max_digits=10, decimal_places=2, default=0
    )
    total_price = models.DecimalField(
        _("السعر الإجمالي"), max_digits=10, decimal_places=2, default=0
    )
    notes = models.TextField(_("ملاحظات"), blank=True)
