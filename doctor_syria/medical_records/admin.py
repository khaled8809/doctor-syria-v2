...
class MedicalRecordAdmin(admin.ModelAdmin):
    ...
    search_fields = ('patient__user__first_name', 'patient__user__last_name', 'title', 'description')
    date_hierarchy = 'date'
    ...
    fieldsets = (
        ...
        (_('تفاصيل السجل'), {
            'fields': ('date', 'severity', 'notes', 'attachments')
        }),
        (_('معلومات النظام'), {
...
class AppointmentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_('معلومات الموعد'), {
            'fields': ('patient', 'doctor', 'appointment_type', 'scheduled_time', 'duration')
        }),
        (_('تفاصيل الموعد'), {
...
class PrescriptionAdmin(admin.ModelAdmin):
    list_display = ('medical_record', 'medicine_name', 'dosage', 'is_chronic', 'refills')
    list_filter = ('is_chronic', 'created_at')
    search_fields = ('medicine_name', 'medical_record__patient__user__first_name')
    readonly_fields = ('created_at', 'updated_at', 'created_by', 'updated_by')
    fieldsets = (
        (_('معلومات الوصفة'), {
            'fields': ('medical_record', 'medicine_name', 'dosage', 'frequency', 'duration')
        }),
        (_('تفاصيل إضافية'), {
...
class VaccinationAdmin(admin.ModelAdmin):
    list_display = ('patient', 'vaccine_type', 'vaccine_name', 'dose_number', 'date_given')
    ...
    fieldsets = (
        ...
        (_('تفاصيل التطعيم'), {
            'fields': ('date_given', 'given_by', 'next_dose_date')
        }),
        (_('معلومات إضافية'), {
            'fields': ('batch_number', 'manufacturer', 'notes')
        }),
        (_('معلومات النظام'), {
            'fields': ('created_at', 'updated_at', 'created_by', 'updated_by'),
            'classes': ('collapse',)
