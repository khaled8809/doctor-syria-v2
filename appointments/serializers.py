...
class AppointmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'appointment_date', 'reason', 'status', 'status_display',
            'priority', 'priority_display', 'notes', 'created_at',
            'updated_at', 'reminder_sent'
        ]
    ...
    def validate_appointment_date(self, value):
        ...
        if doctor:
            ...
            if not schedule:
                raise serializers.ValidationError(_('Doctor is not available on this day'))
            ...
            if not (schedule.start_time <= appointment_time <= schedule.end_time):
                raise serializers.ValidationError(_('Appointment time is outside doctor\'s working hours'))
            ...
            for existing_appointment in existing_appointments:
                if abs((existing_appointment.appointment_date - value).total_seconds()) < schedule.appointment_duration * 60:
    ...
    def validate(self, data):
        ...
        if self.instance and self.instance.status == 'cancelled':
            raise serializers.ValidationError(_('Cannot modify a cancelled appointment'))
