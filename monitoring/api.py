...
from .serializers import SystemMetricSerializer, ErrorLogSerializer, PerformanceLogSerializer
...
class MonitoringViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def system_health(self, request):
        ...
        response_time = metrics.filter(metric_type='response_time').aggregate(Avg('value'))
        
        # حساب معدل الأخطاء
        error_rate = ErrorLog.objects.filter(
            timestamp__gte=last_hour
        ).count() / 3600  # errors per second
    ...
    def performance_stats(self, request):
        ...
        return Response({
            ...
            'error_percentage': (
                ...
                performance_logs.count() * 100
            ) if performance_logs.count() > 0 else 0
        })
