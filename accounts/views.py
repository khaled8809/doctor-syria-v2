        ...
        if form.is_valid():
            try:
                ...
                if totp.verify(token):
                    ...
                    logger.info(f'تم التحقق من المصادقة الثنائية للمستخدم: {user.username}')
                    return redirect('accounts:dashboard')
                else:
...
def update_medical_info(request):
    ...
    try:
        ...
        if form.is_valid():
            ...
            logger.warning(f'خطأ في تحديث المعلومات الطبية للمستخدم: {request.user.username}')
