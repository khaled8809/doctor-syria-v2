...
from .models import (
    Symptom, Disease, DiagnosisSession, DiagnosisResult,
...
class DiagnosisService:
    ...
    def _prepare_symptoms_data(self) -> np.ndarray:
        ...
        for symptom in Symptom.objects.all():
            session_symptom = next(
                (s for s in session_symptoms if s.id == symptom.id),
                None
            )
    ...
    def _format_diagnosis_results(self, predictions: List[Dict[str, float]]) -> List[Dict[str, Any]]:
        """تنسيق نتائج التشخيص"""
        results = []
    ...
    def _generate_reasoning(self, disease: Disease, probability: float) -> Dict[str, Any]:
        """توليد تفسير للتشخيص"""
        session_symptoms = self.session.symptoms.all()
        ...
        for symptom in disease_symptoms:
            if symptom in session_symptoms:
                matching_symptoms.append({
                    'name': symptom.name,
                    'importance': symptom.diseasesymptom_set.get(disease=disease).importance
                })
            else:
