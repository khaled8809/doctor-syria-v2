...
class Hospital(models.Model):
    ...
    logo = models.ImageField(_('شعار المستشفى'), upload_to='hospitals/logos/', blank=True)
    ...
    # إحصائيات
    bed_capacity = models.PositiveIntegerField(_('عدد الأسرّة'), validators=[MinValueValidator(1)])
    icu_capacity = models.PositiveIntegerField(_('عدد أسرّة العناية المركزة'), validators=[MinValueValidator(0)])
    operating_rooms = models.PositiveIntegerField(_('عدد غرف العمليات'), validators=[MinValueValidator(0)])
...
class Room(models.Model):
    ...
    capacity = models.PositiveIntegerField(_('السعة'), validators=[MinValueValidator(1)])
    ...
    def __str__(self):
        return f"{self.room_number} - {self.get_room_type_display()} - {self.department}"
...
class Equipment(models.Model):
    ...
    status = models.CharField(_('الحالة'), max_length=20, choices=EQUIPMENT_STATUS, default='active')
...
class Staff(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='hospital_positions')
