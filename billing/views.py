...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def create_fatura_payment(self, request, pk=None):
        ...
        try:
            ...
            return Response({"payment_url": fatura_data["payment_url"]})
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not FaturaService.verify_webhook_signature(
                signature, timestamp, request.data
            ):
                return Response(
...
def dashboard(request):
    ...
    completed_percentage = (
        (completed_payments / total_payments * 100) if total_payments > 0 else 0
    )
    pending_percentage = (
        (pending_payments / total_payments * 100) if total_payments > 0 else 0
    )

    average_payment = (
        payments.filter(status="completed").aggregate(avg=Avg("amount"))["avg"] or 0
    )
