...
class BackupJob(models.Model):
    ...
    backup_type = models.CharField(
        max_length=20, choices=BACKUP_TYPES, verbose_name=_("نوع النسخ الاحتياطي")
    )
    ...
    completed_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("وقت الإكمال")
    )
    file_path = models.CharField(
        max_length=255, null=True, blank=True, verbose_name=_("مسار الملف")
    )
    file_size = models.BigIntegerField(
        null=True, blank=True, verbose_name=_("حجم الملف")
    )
    error_message = models.TextField(
        null=True, blank=True, verbose_name=_("رسالة الخطأ")
    )
...
class RestorePoint(models.Model):
    ...
    is_automated = models.BooleanField(default=False, verbose_name=_("تلقائي"))
    metadata = models.JSONField(default=dict, verbose_name=_("البيانات الوصفية"))
