...
class TreatmentProgress(models.Model):
    ...
    metrics = models.JSONField(
        default=dict,
        verbose_name=_('مقاييس التقدم')
    )
    next_steps = models.TextField(
        blank=True,
        verbose_name=_('الخطوات التالية')
    )
...
class HealthMetric(models.Model):
    ...
    is_abnormal = models.BooleanField(
        default=False,
        verbose_name=_('قيمة غير طبيعية')
    )
    reference_range = models.CharField(
        max_length=50,
        blank=True,
        verbose_name=_('النطاق المرجعي')
    )
...
class StatisticalReport(models.Model):
    ...
    data = models.JSONField(
        verbose_name=_('البيانات الإحصائية')
    )
    summary = models.TextField(
        verbose_name=_('ملخص التقرير')
    )
    ...
    visualization_settings = models.JSONField(
        default=dict,
        verbose_name=_('إعدادات العرض المرئي')
    )
