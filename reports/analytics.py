...
class AdvancedAnalytics:
    ...
    def analyze_treatment_patterns(diagnosis, min_samples=50):
        ...
        ).values('treatment_plan', 'medications')
        ...
        for patient in Patient.objects.all():
            ...
            if metrics.count() >= min_metrics:
                patients_data.append({
                    ...
                    'avg_visit_duration': appointments.aggregate(
                        Avg('duration')
                    )['duration__avg'] or 0,
                    'conditions_count': reports.values(
                        'diagnosis'
                    ).distinct().count(),
                    'metrics_count': metrics.count(),
                    'last_visit': appointments.aggregate(
                        Max('date')
                    )['date__max']
                })
        ...
        for i in range(3):
            ...
            segments.append({
                ...
                'characteristics': {
                    ...
                    'visit_pattern': 'frequent' if segment_data['visit_frequency'].mean() > df['visit_frequency'].mean() else 'infrequent',
                    'complexity': 'high' if segment_data['conditions_count'].mean() > df['conditions_count'].mean() else 'low'
                }
            })
