...
class ReportingService:
    ...
    def generate_patient_progress_report(patient, start_date=None, end_date=None):
        ...
        # تحليل التقدم
        progress_trend = progress_updates.values('status').annotate(
            count=Count('id')
        ).order_by('-count')
        ...
        return {
            ...
            'progress': {
                ...
                'latest_status': progress_updates.latest('date').status if progress_updates.exists() else None,
            },
            'reports_summary': {
                'total': reports.count(),
    ...
    def generate_clinic_statistics(start_date=None, end_date=None):
        ...
        return {
            ...
            'appointments': {
                ...
                'daily_average': appointments.count() / ((end_date - start_date).days or 1),
            },
            'reports': {
                'total': reports.count(),
    ...
    def analyze_treatment_effectiveness(diagnosis, start_date=None, end_date=None):
        ...
        progress_data = TreatmentProgress.objects.filter(
            report__in=reports
        ...
        return {
            ...
            'common_treatments': pd.Series(
                ' '.join(treatment_plans).split()
            ).value_counts().head(10).to_dict(),
            'common_medications': pd.Series(
                ' '.join(medications).split()
            ).value_counts().head(10).to_dict(),
        }
    ...
    def generate_health_trends(metric_type, period='monthly'):
        ...
        df = pd.DataFrame(
            metrics.values('measured_at', 'value', 'patient')
        )
        if not df.empty:
