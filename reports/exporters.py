...
class ReportExporter:
    ...
    def export(self, format_type):
        ...
        if format_type not in self.EXPORT_FORMATS:
            raise ValueError(f"تنسيق غير مدعوم. التنسيقات المدعومة: {', '.join(self.EXPORT_FORMATS)}")
    ...
    def _export_pdf(self, filename):
        ...
        HTML(string=html_content).write_pdf(
            output_file,
            stylesheets=[
                os.path.join(settings.STATIC_ROOT, 'css/report_pdf.css')
            ]
        )
    ...
    def _export_csv(self, filename):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8-sig') as csvfile:
            if isinstance(self.report_data, dict):
                ...
                if self.report_data:
                    writer = csv.DictWriter(csvfile, fieldnames=self.report_data[0].keys())
                    writer.writeheader()
